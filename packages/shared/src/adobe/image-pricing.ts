 ...
 * per-model 倍率由系统设置提供（IMAGE_MODEL_MULTIPLIERS）；未知模型恒返回 1。
...
// 从 `firefly-<family>...` 或兼容的裸 `nano-banana...` 模型 id 中按"最长前缀"匹配
// 出族，避免 nano-banana 误吞 nano-banana-pro / nano-banana2，gpt-image-2 误吞
// gpt-image-1.5。裸 gpt-image 继续保持普通模型计费口径。
 ...
 * @param model 模型 id；支持 `firefly-<family>[-<...>]` 与裸 `nano-banana...`。
 * @returns 命中的模型族；无法匹配时返回 null。
 */
function resolveFireflyImageFamily(
  ...
  const hasFireflyPrefix = normalized.startsWith(FIREFLY_MODEL_PREFIX);
  const rest = hasFireflyPrefix
    ? normalized.slice(FIREFLY_MODEL_PREFIX.length)
    : normalized;
  ...
  for (const family of byLength) {
    if (!hasFireflyPrefix && !family.startsWith("nano-banana")) continue;
...
/**
 * 解析某图像模型的计费倍率。
 * @param model 图像模型 id（如 `firefly-gpt-image-2` 或 `nano-banana-pro`）。
 * @param multipliers family → 倍率 的配置 map（来自 IMAGE_MODEL_MULTIPLIERS）。
 * @returns 命中族且倍率为正有限数则返回该倍率；否则返回 1。
 */
