---
title: Adobe Firefly 兼容转换
description: 站内标准请求（prompt / size / quality / model / images）如何被兼容转换成 Adobe Firefly 接受的格式。
---
...
| 图生图 `images[]` | gpt-image 族：`referenceBlobs[{ id, usage:"subject" }]`；nano-banana 族：`referenceBlobs[{ id, usage:"general" }]` | 先 `uploadImage` 拿 Adobe image id，再放入 referenceBlobs（新 API 已拒收 referenceImages）。两族 usage 恰好相反、不可共用；普通/未知模型默认落 gpt-image-2，故默认用 `subject` |
...
- 路由与兜底调度（adobe 如何挂入分组、按 priority 兜底）：[Adobe 路由与兜底调度](/docs/adobe-firefly-routing)
