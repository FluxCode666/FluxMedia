...
- **图像与视频固定价格计费**：图片按实际输出最长边归入 1024、1K、2K、4K 固定档位，逐档读取分组模型覆盖、全局模型价格和通用价格，再叠加实际发生且允许为 0 的审核费用；视频改为模型族每秒固定积分乘时长。模型、分组与 Adobe 后端倍率已退出配置、展示和运行时计费，旧视频模型倍率会按原基础每秒价自动迁移为固定每秒价格。下方旧版本中的倍率说明仅记录当时行为。
