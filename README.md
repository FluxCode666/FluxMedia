...
README 只保留项目入口和最小部署说明。接口字段、后端调度、计费、Agent、队列、Sub2API、管理员权限等细节以站内系统文档为准：

- 公开系统文档：`/docs/system`
- 登录后台文档：`/dashboard/backend-help`
- 脱敏部署 Runbook：[docs/deploy-superapi.md](docs/deploy-superapi.md)
...
- 真实 `.env`、HAR 抓包、rollout 日志、运维记忆、API Key、数据库地址和用户数据不要提交到仓库。

## 生产部署

生产至少需要三个模块：Web 应用、Go ChatGPT Web sidecar、crontab HTTP 任务。
...
CRON_SECRET=...
NEXT_PUBLIC_ASSET_PREFIX=/next-assets-<build-id>
```

如果使用 Nginx 静态 alias 或 AB 上线，每次构建更换 `NEXT_PUBLIC_ASSET_PREFIX`，先同步 `apps/web/.next/static` 到对应静态目录，再切换公网流量，避免旧 chunk 404。

### 2. Go Sidecar

Web 账号池依赖 `services/chatgpt-web-proxy`，生产环境应单独常驻运行。
...
除上述密钥和域名外，尽量复用系统初始化写入的默认配置；启动后通过后台系统设置调整，不要把线上私有配置写进仓库。
