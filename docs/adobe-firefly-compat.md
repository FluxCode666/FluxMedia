...
```
最终图像积分 = 固定模型/档位基础价 + 运行时审核费
```

- 固定档位取**实际输出像素的最长边**：小于 1248px 为 1024；1248px 至小于 2048px 为 1K；2048px 至小于 3840px 为 2K；达到或超过 3840px 为 4K。
- 模型价格优先级：选中分组的模型逐档覆盖 → 全局模型固定价格。全局模型四档价格必填；分组覆盖是固定价格，不是倍率。
- 审核费从运行时设置读取：文本审核按请求计，输入图片审核按张计。关闭内容审核时不执行审核也不收费；审核开启时两项费用均可配置为 0。

视频总积分：

```
总积分 = ceil2(当前视频模型族每秒积分 × 时长秒数)
```

每个视频模型族必须配置全局每秒积分，分组可按模型族覆盖；分组未配置时继承全局价格。视频价格不再叠加模型、分组或 Adobe 后端倍率。

### 算例：nano-banana-pro、2K、混合分组

- 实际输出为 `2048x2048`，最长边 2048px，使用 2K 档。
- 先查混合分组对 `nano-banana-pro` 的 2K 覆盖；未配置则查全局该模型的 2K 价格；再未配置才使用通用 2K 价格。
- 最终积分为命中的固定 2K 基础价，加上本请求实际发生的文本审核费和输入图片审核费；不再叠加图像模型、Adobe 或分组倍率。
